...
| `GET /call/audio` | `name`,`loop=0/1` / `stop=1` / `seek`(秒) | 通话中播放乐库音频（SCO 下行模拟对端说话）。参数优先级 `stop`>`seek`>`name`；`seek` 拖进度（钳制 `[0,dur]`），未播放时报错、不误起播。**多路**：音频绑定到当前 active 那路，需先接通；每路可各绑不同文件，通话切换时车机听到的"对端"自动跟随（续各自进度，`CALL_AUDIO_FOLLOW` 事件）；`stop` 解绑该路 | 双 |
| `GET /call/audio-status` | — | 一行通话音频进度（上位机进度条 1s 轮询）：`callAudio=off` 或 `callAudio=playing name="xx.mp3" pos=12s dur=35s loop=0 num=138…`。name 为纯文件名（无"(循环)"后缀，循环态用 `loop=1` 表达）；`num=`=当前出声的是哪路通话（多路跟随的可视判据） | 双 |
