...
- [ ] **多路通话车机显示（第八轮新增，PC 侧已全验）**：
  - [ ] 通话中注入第二路：车机应显示**等待来电**（CCWA 语义，双路 CLCC）——`incoming` 后看车机屏；
  - [ ] 车机上直接按接听接等待电话（不经 PC `answer`）：应两路并存、原路显示保持中（走 `onAnswer` 编排）；
  - [ ] 车机 CHLD 切换键：应互换 active/held（走 `onHold`/`onUnhold` 回调，事件流看 `CAR_HOLD`/`CAR_UNHOLD`）；
  - [ ] 挂一路后车机剩余通话显示：挂 active 后另一路应在 ~0.3s **自动恢复通话**（CHLD=1 网络取回，第十一轮纠正+第十二轮时延化）；挂 held 不影响 active；
  - [ ] 第三路来电（PC 注入被拒）：车机侧应无任何反应（GSM 网络侧本来就进不来）。
- [ ] **第十二轮：拆线竞态 + 通话音频路由（台架复测重点，两个实测 bug 的回归）**：
  - [ ] A active + B held，车机上把两路都挂掉：应全部清干净，**不得再冒出挂不掉的"僵尸通话"**（原事故场景）；
  - [ ] 拨出后 3s 内在车机上取消拨号：3s 后不得出现"自动接通"幽灵事件，状态归 idle；
  - [ ] 通话中车机界面切"声音→手机"：车机静音、手机听筒出声；手机上切扬声器/听筒同理；切回蓝牙恢复（事件流看 `CALL_AUDIO_ROUTE`）；
  - [ ] 两路通话各绑音频：接通后车机应能听到（原"进度在走没声音"——接通显式请求蓝牙路由 + SCO 迟到重试）；
  - [ ] 单曲播放列表播放中车机按下一曲：应**从头重播**（不是续播）。
...
（配对/音乐/来电/通讯录/断连回连）、第五轮 exe 打包与发布、第六轮稳固轮（静音流忙循环
事故——TECH §5）、第七轮通话音频进度接口、第八轮多路通话（呼叫等待/保持切换/三方接听）。
