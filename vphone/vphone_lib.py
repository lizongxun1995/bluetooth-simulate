...
一台 Android 手机装上 vphone APK 后, 即成为 PC 可远程驱动的蓝牙测试终端:
  ...
  门C(电话): incoming/dial/answer/hangup/hold/swap/dtmf/audio_bt + call_audio(通话中自定义音频)
             ...
             各路独立"对端音频"(切换跟随, HFP 单 SCO 同真机; 路由跟随 蓝牙/听筒/扬声器 CALL_AUDIO_ROUTE)
             /挂 active 后保持路自动恢复(真机CHLD=1, 延迟300ms重验防拆线竞态)
...
__version__ = "0.8.4"   # 单一版本真源: pyproject.toml 动态引用此处(dynamic attr)
