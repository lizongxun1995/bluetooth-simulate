...
| 音乐(A2DP/AVRCP) | 元数据任意伪造；电脑音频上传→车机真实出声；静音流保活模式；来电自动暂停/挂断自动续播(真机焦点) | ✅ |
| 车机按键回流 | 播放/暂停/切歌/拖进度/接听/挂断/拨号 全部落结构化事件可断言 | ✅ |
| 通话(HFP) | 注入来电/拨号/保持/DTMF/通话中自定义音频(对端说话)；多路等待/切换、挂 active 自动恢复保持路 | ✅ |
...
# 开发机(三种任选):
pip install vphone/dist_py/vphone-0.8.2-py3-none-any.whl   # 装库+CLI+GUI 命令, 零第三方依赖
cd vphone
